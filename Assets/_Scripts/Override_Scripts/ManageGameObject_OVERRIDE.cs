...
namespace UnityMcpBridge.Editor.Tools
{
    ...
    public static class ManageGameObject
    {
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                switch (action)
                {
                    ...
                    case "move":
                        return MoveGameObject(@params, targetToken, searchMethod);
                    case "modify":
        ...
        private static object DuplicateGameObject(JObject @params, JToken targetToken, string searchMethod)
        {
            ...
            try
            {
                // Get parameters for the duplicate
                string newName = @params["name"]?.ToString(); // Optional new name
                Vector3? position = ParseVector3(@params["position"] as JArray); // Optional position offset
                ...
                {
                    duplicateGo.name = newName;
                }
                else
                {
                    duplicateGo.name = $"Clone {sourceGo.name}";
                }

                // Apply position offset - if provided use that, otherwise use default offset
                Vector3 finalOffset;
                if (position.HasValue)
                {
                    finalOffset = position.Value;
                }
                else
                {
                    // Default offset: 1 unit to the right and slightly up
                    finalOffset = new Vector3(1f, 0.1f, 0f);
                }
                duplicateGo.transform.position = sourceGo.transform.position + finalOffset;

                // Maintain same parent as original
                duplicateGo.transform.SetParent(sourceGo.transform.parent, true);
                ...
                return Response.Success(
                    $"GameObject '{sourceGo.name}' duplicated successfully as '{duplicateGo.name}'.",
                    Helpers.GameObjectSerializer.GetGameObjectData(duplicateGo)
                );
            }
            catch (Exception e)
            {
        ...
        private static object MoveGameObject(JObject @params, JToken targetToken, string searchMethod)
        {
            ...
            try
            {
                // Get the target GameObject (the reference object)
                string targetName = @params["targetObject"]?.ToString();
                if (string.IsNullOrEmpty(targetName))
                {
                    return Response.Error("Target GameObject name is required for move operation.");
                }

                GameObject targetGo = FindObjectInternal(new JValue(targetName), "by_name");
                if (targetGo == null)
                {
                    return Response.Error($"Target GameObject '{targetName}' not found.");
                }
                ...
                sourceGo.transform.position = newPosition;

                // Mark the scene as dirty
                EditorUtility.SetDirty(sourceGo);

                return Response.Success(
                    $"GameObject '{sourceGo.name}' moved {relativePosition} '{targetGo.name}'.",
                    Helpers.GameObjectSerializer.GetGameObjectData(sourceGo)
                );
            }
            catch (Exception e)
            {
                return Response.Error($"Error moving GameObject '{sourceGo.name}': {e.Message}");
            }
        }
        ...
        private static Bounds GetObjectBounds(GameObject go)
        {
            ...
            // If no renderers or colliders, use a default size
        ...
        private static object ScaleGameObject(JObject @params, JToken targetToken, string searchMethod)
        {
            ...
            try
            {
                Vector3 currentScale = targetGo.transform.localScale;
                Vector3 newScale = currentScale;
                string scaleCommand = @params["scaleCommand"]?.ToString().ToLower();
                float? multiplier = @params["multiplier"]?.ToObject<float>();
                Vector3? exactScale = ParseVector3(@params["scale"] as JArray);

                // Handle exact scale if provided
                if (exactScale.HasValue)
                {
                    newScale = exactScale.Value;
                }
                // Handle natural language commands
                else if (!string.IsNullOrEmpty(scaleCommand))
                {
                    ...
                    else if (scaleCommand.Contains("times") && multiplier.HasValue)
                    {
                        newScale = currentScale * multiplier.Value;
                    }
                }
                else
                {
                ...
                newScale.x = Mathf.Max(newScale.x, 0.0001f);
                newScale.y = Mathf.Max(newScale.y, 0.0001f);
                newScale.z = Mathf.Max(newScale.z, 0.0001f);
            ...
            {
                return Response.Error($"Error scaling GameObject '{targetGo.name}': {e.Message}");
            }
        }
